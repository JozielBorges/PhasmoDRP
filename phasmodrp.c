...
{ if(dsc && dsc->core) dsc->core->run_callbacks(dsc->core); }
...
#define IL2CPP_API(a,b,c,d,e) typedef a (*p_##d) b;                                    \
                                static p_##d d = NULL;                                 \
                                if(!d && !(d = (p_##d)GetProcAddress(c, #d))) return e
...
#define UNITY_API(a,b,c,d) typedef a (__thiscall *p_##d) b;                    \
                             static p_##d d = NULL;                            \
                             if(!d && !(d = (p_##d)GetMethodAddress c)) return

HMODULE hGameAssembly = NULL;
FARPROC oriLateUpdate = NULL;

void myLateUpdate(void)
{
	oriLateUpdate();
	...
	if(current_time - last_time > 500) {
		...
		static Discord *dsc = NULL;
		if(!dsc && !(dsc = Discord_Initialize(DiscordID))) return;
		...
		if(GetInRoom() && (Room = GetRoom())) {
			...
			if(PlayerCount != memPlayerCount) { changed = TRUE; memPlayerCount = PlayerCount; }
			if(MaxPlayers != memMaxPlayers) { changed = TRUE; memMaxPlayers = MaxPlayers; }
			...
			if(SceneName && wcscmp(SceneName->s, memSceneName)) {
				...
				if(!wcscmp(memSceneName, L"Menu_New")) wcscpy_s(fmtSceneName, _countof(fmtSceneName), L"Lobby");
					...
					for(SIZE_T i = 0; i < _countof(memSceneName) && memSceneName[i]; i++)
						if(!(fmtSceneName[i] = memSceneName[i] != '_' ? memSceneName[i] : ' ')) break;
				}
				timeRoom = time(NULL);
			}

			if(!memInRoom || changed) {
				memInRoom = TRUE;
				if(!changed) timeRoom = time(NULL);
	...
	if(GamePID) {
		...
		if(hProc) {
			...
			if(GetModuleFileName(hDLL, path, _countof(path))) {
				...
				if(mem) {
					WriteProcessMemory(hProc, mem, path, sizeof(path), NULL);
